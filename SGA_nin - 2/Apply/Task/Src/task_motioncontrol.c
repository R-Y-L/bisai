...
void Task_Motion_Process(void)
{
	...
	//将反�?的PWM输出增大�?倍数
	//Thruster_nagative_data_handle(2.3);
...
void HandleMode_data_handle()
{
	...
	if(x_y_z_pitch & 0x08)
		z_Speed += speed_kV * MAX_VPOWER / 100 + 30 ;
	if(x_y_z_pitch & 0x04)
	    z_Speed -= speed_kV * MAX_VPOWER / 100 - 50 ;
	...
	if(x_y_z_pitch & 0x80)
		{
			PWMInfo.PWMout[v_wheel3_speed] -= 20;
		}
	else if(x_y_z_pitch & 0x40)
		{
			PWMInfo.PWMout[v_wheel1_speed] -= 20;
		}
