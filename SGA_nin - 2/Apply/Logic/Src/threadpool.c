...
/*void DepthControl(void* paramenter)
{
    ...
    while(1)
    {
        //瀹氭繁鏁版嵁娑堟伅闃熷垪鎺ユ敹鍒版暟锟�?锛屽皢寮€濮嬪畾娣辨帶锟�?
        ...
        Drv_Delay_Ms(600);    //姣忛殧涓€娈垫椂闂磋繘琛屼竴娆″畾锟�?
    }
}*/
...
{
	PWMInfo.PWMout[claw_shouder_Speed] = Servo_Angle_To_HightTime(CLAW_ELBOW_STOP_ANGLE);
 ...
 //   State_control |= 0x10;
    ...
    {		
        ...
        {
            PWMInfo.PWMout[claw_shouder_Speed] += 15;
            PWMInfo.PWMout[claw_elbow_Speed] += 15;
        ...
        {
            PWMInfo.PWMout[claw_shouder_Speed] -= 15;
            PWMInfo.PWMout[claw_elbow_Speed] -= 20;
            if(PWMInfo.PWMout[claw_catchSpeed] < 1435)
                PWMInfo.PWMout[claw_catchSpeed] = 1435;
            if(PWMInfo.PWMout[claw_shouder_Speed] < Servo_Angle_To_HightTime(CLAW_SHOUDER_STOP_ANGLE))
                PWMInfo.PWMout[claw_shouder_Speed] = Servo_Angle_To_HightTime(CLAW_SHOUDER_STOP_ANGLE);
            // if(PWMInfo.PWMout[claw_shouder_Speed] > Servo_Angle_To_HightTime(CLAW_SHOUDER_STOP_ANGLE-40))
            //     PWMInfo.PWMout[claw_elbow_Speed] = Servo_Angle_To_HightTime(CLAW_SHOUDER_STOP_ANGLE-40);
        }
		//灯光亮暗控制
        if(right_rocker == 3)
        ...
        {
            PWMInfo.PWMout[light_level_1] -= 200;
            if(PWMInfo.PWMout[light_level_1] < 1050) 
            PWMInfo.PWMout[light_level_1] = 1050;
            right_rocker = 0;
        }
        //摄像头云台
